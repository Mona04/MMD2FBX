...
namespace Framework
{
	...
	public:
		...
		template <typename T, typename = typename std::enable_if <
			...
			std::is_same<T, uint8_t>::value ||
			std::is_same<T, UInt8Vector2>::value
		> ::type > void Write(const std::vector<T>& value)
		{
			uint length = static_cast<uint>(value.size());
			Write(length);

			out.write(reinterpret_cast<const char*>(value.data()), sizeof(T) * length);
		}
		...
		template <typename T, typename = typename std::enable_if <
			...
			std::is_same<T, uint8_t>::value ||
			std::is_same<T, UInt8Vector2>::value
		> ::type > void Read(std::vector<T>& value)
		{
			uint length = ReadUInt();

			value.clear(); value.shrink_to_fit();
			value.reserve(length); value.resize(length);

			in.read(reinterpret_cast<char*>(value.data()), sizeof(T) * length);
		}
